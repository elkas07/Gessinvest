...
const App: React.FC = () => {
  ...
  const renderAdminContent = () => {
    switch (activeTab) {
        ...
        return (
          <div className="space-y-8 animate-fade">
            <div className="flex justify-between items-center">
              <div>
                <h2 className="text-4xl font-black tracking-tighter uppercase italic text-slate-900">Console Admin GESS</h2>
                <p className="text-slate-400 font-bold uppercase text-[10px] tracking-[0.3em]">Gestion globale du réseau d'investissement</p>
              </div>
              <button 
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              {[
                { label: "Volume Sous Gestion", val: formatCFA(projects.reduce((acc, p) => acc + (p.collectedAmount || 0), 0)), color: "text-[#ff6b35]" },
                { label: "Investisseurs", val: allUsers.length, color: "text-slate-900" },
            ...
            <div className="bg-white rounded-[2rem] border border-slate-100 overflow-hidden shadow-sm">
              <DataTable title="Flux financiers récents" data={transactions.slice(0, 10)} columns={[
                { header: 'Utilisateur', render: (t) => <span className="font-bold text-sm">{t.userName || 'Membre GESS'}</span> },
      ...
      case 'users':
        return (
          <div className="space-y-6 animate-fade">
            <DataTable 
              title="Portefeuilles Investisseurs"
      ...
      case 'kyc':
        return (
          <div className="space-y-6 animate-fade">
            <DataTable title="Vérifications d'identité" data={allUsers.filter(u => u.kycStatus === 'pending')} columns={[
              { header: 'Candidat', render: (u) => <div className="font-bold text-sm">{u.name}</div> },
          ...
          <div className="space-y-8 animate-fade text-center py-20">
             <div className="max-w-xl mx-auto bg-[#0d1b2a] p-12 rounded-[3rem] text-white shadow-2xl relative overflow-hidden">
                <div className="relative z-10">
                  <h3 className="text-3xl font-black mb-6 tracking-tighter uppercase italic">Distributions ROI</h3>
                  <p className="text-slate-400 text-sm font-medium mb-10">Déclenchement du versement des dividendes mensuels pour tous les copropriétaires GESS.</p>
  ...
  const renderInvestorContent = () => {
    if (selectedProject) {
      return (
        <div className="animate-fade pb-20">
          <button onClick={() => setSelectedProject(null)} className="mb-8 flex items-center gap-2 text-slate-400 hover:text-slate-900 font-black text-xs uppercase tracking-widest transition-all">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            <div className="lg:col-span-2 space-y-10">
              ...
              <div className="bg-white p-12 rounded-[3rem] border border-slate-100 shadow-sm space-y-8">
                ...
                <div className="grid grid-cols-3 gap-8 pt-10 border-t">
                  ...
                  <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Collecté</p>
                    <p className="text-xl font-black text-[#ff6b35]">{formatCFA(selectedProject.collectedAmount)}</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Durée</p>
            ...
            <div className="space-y-8">
              <div className="bg-[#0d1b2a] text-white p-10 rounded-[3rem] shadow-2xl sticky top-8">
                <h4 className="text-xl font-black mb-10 tracking-tight uppercase italic underline decoration-[#ff6b35] decoration-4">Investir dans cet actif</h4>
                <div className="space-y-8">
                  <div>
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4 block">Montant souhaité (F CFA)</label>
                    <input type="number" defaultValue="250000" className="w-full bg-white/5 border border-white/10 p-6 rounded-[1.5rem] font-black text-2xl outline-none focus:border-[#ff6b35] transition-all shadow-inner text-white" />
                  </div>
                  ...
                  <p className="text-[10px] text-center text-slate-500 font-medium">Fonds sécurisé par GESS INVEST.</p>
                </div>
              </div>
            </div>
          </div>
    ...
    switch (activeTab) {
        ...
        return (
          <div className="space-y-10 animate-fade pb-20">
             <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-2 bg-[#0d1b2a] rounded-[3rem] p-14 text-white relative overflow-hidden shadow-2xl">
                <div className="relative z-10 flex flex-col h-full justify-between">
                  <div>
                    <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.4em] mb-4">Solde de votre portefeuille</p>
              ...
              <div className="bg-white rounded-[3rem] p-12 border border-slate-100 flex flex-col justify-between shadow-sm">
                <div>
                  <p className="text-slate-400 text-[10px] font-black uppercase mb-4 tracking-widest">Gains générés (Total)</p>
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-100 overflow-hidden shadow-sm">
               <DataTable title="Historique des transactions" data={transactions.filter(t => t.userId === currentUser?.id).slice(0, 5)} columns={[
      ...
      case 'investments':
        return (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 animate-fade pb-20">
            {projects.map(p => (
              <div key={p.id} onClick={() => setSelectedProject(p)} className="bg-white rounded-[3rem] overflow-hidden border border-slate-100 hover:shadow-2xl transition-all group shadow-sm flex flex-col h-full cursor-pointer hover:-translate-y-2">
                ...
                <div className="p-12 flex-1 flex flex-col">
                  ...
                  <div className="mt-auto space-y-8">
                    ...
                    <button className="w-full py-6 bg-[#0d1b2a] text-white rounded-[1.25rem] font-black text-[12px] uppercase tracking-widest group-hover:bg-[#ff6b35] transition-all shadow-xl">Voir l'actif</button>
                  </div>
                </div>
              </div>
            ))}
      ...
      case 'simulator':
        return (
          <div className="max-w-2xl mx-auto py-10 animate-fade text-center pb-20">
            <div className="bg-white p-16 rounded-[4rem] border border-slate-100 shadow-2xl relative overflow-hidden">
                ...
                <h3 className="text-5xl font-black text-slate-900 tracking-tighter mb-4">Calculateur GESS</h3>
                <p className="text-slate-400 font-bold uppercase text-[10px] tracking-[0.4em] mb-16">Projetez vos gains passifs</p>
      ...
      case 'profil':
        return (
          <div className="max-w-md mx-auto py-10 space-y-10 animate-fade text-center pb-20">
             ...
             <div className="bg-white p-12 rounded-[3rem] border border-slate-100 shadow-sm">
                <h4 className="text-[11px] font-black uppercase text-slate-400 mb-8 tracking-widest text-left">Sécurité KYC</h4>
                <div className="border-3 border-dashed border-slate-100 p-14 rounded-[2.5rem] text-center text-slate-400 text-xs font-black hover:border-[#ff6b35] hover:bg-[#ff6b35]/5 transition-all cursor-pointer group shadow-inner">
                   {currentUser?.kycStatus === 'verified' ? 'VÉRIFICATION TERMINÉE' : 'TÉLÉCHARGER PIÈCE D\'IDENTITÉ'}
                </div>
             </div>
          </div>
        );
    ...
    return (
      <div className="min-h-screen bg-white flex flex-col">
        <nav className="h-28 border-b flex items-center justify-between px-10 md:px-32 bg-white sticky top-0 z-50">
          ...
          <button onClick={() => setView('app')} className="bg-[#0d1b2a] text-white px-12 py-5 rounded-[1.25rem] font-black text-[12px] uppercase tracking-widest shadow-2xl hover:bg-[#ff6b35] transition-all">Connexion Investisseur</button>
        </nav>
        <div className="flex-1 flex flex-col items-center justify-center px-10 text-center py-40 bg-[#fcfcfc]">
          <h1 className="text-[6rem] md:text-[10rem] font-black text-[#0d1b2a] tracking-tighter mb-16 leading-[0.8] animate-fade">
            Investir au <br/><span className="text-[#ff6b35] italic">Tchad.</span>
          </h1>
          <p className="text-slate-400 text-2xl font-medium max-w-3xl mx-auto mb-20 leading-relaxed tracking-tight">
            Devenez copropriétaire d'actifs immobiliers de prestige à partir de <span className="text-[#0d1b2a] font-black italic underline decoration-[#ff6b35] decoration-4">10.000 F CFA</span>.
          </p>
          <button onClick={() => setView('app')} className="bg-[#ff6b35] text-white px-20 py-8 rounded-[1.5rem] font-black text-[14px] uppercase tracking-widest shadow-2xl hover:scale-105 transition-all">Ouvrir mon compte</button>
        </div>
      </div>
    );
  }
      ...
      <div className="pb-52">
        {loading ? (
          <div className="flex flex-col items-center justify-center h-[60vh] gap-10">
            ...
            <p className="text-[12px] font-black text-[#0d1b2a] uppercase tracking-[0.6em] animate-pulse">CHARGEMENT GESS INVEST...</p>
          </div>
        ) : (
      ...
      {showBalanceModal && (
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-xl z-[9999] flex items-center justify-center p-8">
          <div className="bg-white w-full max-w-md rounded-[4rem] p-16 shadow-2xl animate-fade border border-slate-100">
            <h3 className="text-3xl font-black mb-10 tracking-tighter uppercase italic text-slate-900 underline decoration-[#ff6b35] decoration-4">Gérer Solde</h3>
            <div className="mb-10 p-6 bg-slate-50 rounded-3xl flex items-center gap-5">
               <div className="w-14 h-14 bg-[#0d1b2a] rounded-2xl flex items-center justify-center text-white font-black text-2xl">{showBalanceModal.name.charAt(0)}</div>
            ...
            <div className="space-y-6 mb-12">
               <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest block ml-2">Ajustement (F CFA)</label>
               <input type="number" value={balanceAdjustment} onChange={(e) => setBalanceAdjustment(Number(e.target.value))} className="w-full bg-slate-50 border-3 border-slate-100 p-7 rounded-3xl font-black text-3xl outline-none focus:border-[#ff6b35] transition-all shadow-inner" placeholder="0" />
            </div>
            <div className="flex gap-6">
              <button onClick={() => setShowBalanceModal(null)} className="flex-1 text-slate-400 font-black text-[12px] uppercase tracking-widest hover:text-slate-900 transition-colors">Fermer</button>
      ...
      {showAddProjectModal && (
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-xl z-[9999] flex items-center justify-center p-8">
          <div className="bg-white w-full max-w-2xl rounded-[4rem] p-16 shadow-2xl animate-fade border border-slate-100 overflow-y-auto max-h-[90vh] custom-scrollbar">
            <h3 className="text-4xl font-black tracking-tighter uppercase italic text-slate-900 underline decoration-[#ff6b35] decoration-4 mb-12">Nouveau Projet</h3>
            <div className="grid grid-cols-2 gap-8 mb-12">
               <div className="col-span-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block ml-2">Titre du projet</label>
                  <input type="text" className="w-full bg-slate-50 border-2 border-slate-100 p-5 rounded-2xl font-bold focus:border-[#ff6b35] outline-none transition-all shadow-inner" placeholder="Ex: Cité des Stars" />
               </div>
               <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block ml-2">Cible (F CFA)</label>
               ...
               <div className="col-span-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block ml-2">Description complète</label>
                  <textarea className="w-full bg-slate-50 border-2 border-slate-100 p-5 rounded-2xl font-medium h-40 focus:border-[#ff6b35] outline-none transition-all shadow-inner" placeholder="Détails de l'investissement..."></textarea>
               </div>
            </div>
