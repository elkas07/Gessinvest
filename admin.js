...
async function afficherMembres(filtre = '') {
  try {
    ...
    // Appliquer le filtre
    const membresFiltres = filtre ? 
      membres.filter(m => 
        m.nom.toLowerCase().includes(filtre.toLowerCase()) || 
        (m.telephone && m.telephone.includes(filtre))
      ) : 
      membres;
    ...
    membresFiltres.forEach(membre => {
      ...
      // Ajouter les boutons d'action selon le statut
